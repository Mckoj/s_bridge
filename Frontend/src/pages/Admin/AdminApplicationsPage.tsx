      ...
      <div className="max-w-7xl mx-auto space-y-6 pb-12">
        ...
        {!loading && !error && filtered.length === 0 && (
          <EmptyState
            ...
            description={search ? "No application records match your search query." : "No student applications found in the database."}
          />
        )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-800/40 text-xs">
                  {filtered.map((app) => (
                    <tr key={app.id} className={`transition-colors ${dark ? "hover:bg-slate-800/40" : "hover:bg-slate-50"}`}>
                      <td className="p-4">
                        <p className={`font-bold ${dark ? "text-white" : "text-slate-800"}`}>
                          {app.studentName}
                        </p>
                        {app.programme && (
                      ...
                      <td className={`p-4 font-medium ${dark ? "text-slate-300" : "text-slate-600"}`}>
                      ...
                      <td className="p-4">
                        <span className={`text-[10px] font-bold px-2.5 py-1 rounded-md border ${
                          ...
                          app.status === "REVIEWING" ? "bg-purple-500/10 text-purple-400 border-purple-500/20" :
