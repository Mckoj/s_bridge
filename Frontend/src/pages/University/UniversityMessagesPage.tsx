        ...
        {loading ? (
          ...
          <div
            className={`rounded-3xl border p-12 text-center shadow-xl ${
              dark ? "bg-slate-900/80 border-slate-800" : "bg-white border-violet-200/80"
            }`}
          >
            ...
            <p className="text-xs text-slate-400 max-w-sm mx-auto mt-1">
              Direct message conversations will appear here when students or company recruiters send messages.
            </p>
          </div>
        ) : (
          <div
          ...
          >
            {/* Threads Sidebar */}
            <div className={`border-r p-4 space-y-4 flex flex-col ${dark ? "border-slate-800 bg-slate-950/40" : "border-slate-100 bg-slate-50/60"}`}>
              ...
              <div className="space-y-1 overflow-y-auto flex-1">
                {filteredConversations.map((t) => {
                  const isSelected = t.id === activeConvId;
                  return (
                    <div
                      key={t.id}
                    ...
                    >
                      <div className={`w-9 h-9 rounded-xl font-bold text-xs flex items-center justify-center ${isSelected ? "bg-white/20 text-white" : "bg-violet-500/20 text-violet-400"}`}>
                        {t.partner.avatar}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <p className="text-xs font-bold truncate">{t.partner.name}</p>
                          {t.unreadCount > 0 && (
                            <span className="bg-violet-400 text-slate-950 text-[9px] font-extrabold px-1.5 py-0.5 rounded-full">
                              {t.unreadCount}
                            </span>
                          )}
                        </div>
                        <p className={`text-[10px] truncate ${isSelected ? "text-violet-200" : "text-slate-400"}`}>{t.partner.role}</p>
                        <p className={`text-[11px] truncate mt-0.5 ${isSelected ? "text-white/80" : "text-slate-400"}`}>{t.lastMessage}</p>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="md:col-span-2 flex flex-col justify-between p-6">
              {activeConv ? (
                <>
                  <div className="pb-4 border-b border-slate-800/40 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[11px] text-violet-400 font-semibold">{activeConv.partner.role}</p>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleSend} className="pt-3 border-t border-slate-800/40 flex gap-2">
                    ...
                    <button
                      ...
                      className="px-5 py-2.5 rounded-xl bg-violet-600 hover:bg-violet-700 disabled:opacity-50 text-white text-xs font-bold shadow-lg shadow-violet-500/20 flex items-center gap-1 cursor-pointer"
                    >
                      <Send size={14} /> Send
                    </button>
                  </form>
                ...
                <div className="flex-1 flex items-center justify-center text-xs text-slate-500">
                  Select a contact thread to view conversation.
                </div>
              )}
            </div>
          </div>
