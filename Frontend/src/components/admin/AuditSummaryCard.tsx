...
export default function AuditSummaryCard({
  ...
  const displayValue = unavailable || value === undefined || value === null ? "—" : value;
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="flex items-baseline gap-2 mt-2">
            <p className={`text-3xl font-black tabular-nums ${dark ? "text-white" : "text-slate-900"}`}>
              {displayValue}
            </p>
            {unavailable && (
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${
                dark ? "bg-slate-800 text-slate-400 border border-slate-700" : "bg-slate-100 text-slate-500 border border-slate-200"
              }`}>
                N/A
              </span>
            )}
