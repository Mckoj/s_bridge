...
// Format a number nicely: 1240 → "1,240", 0 → "0"
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    gsap.set(
      [headingRef.current, subRef.current, ctaRef.current, statsRef.current, logoRef.current],
      { opacity: 0 }
    );
  ...
  // Stat cards — values come from backend, with skeleton while loading
  const statCards = [
    {
      value: loading ? null : fmt(stats?.activePlacements),
      label: "Active Placements",
      color: "text-blue-300",
      bg: "bg-blue-500/10",
    },
    {
      value: loading ? null : fmt(stats?.verifiedEmployers),
  ...
  return (
    <section
      id="home"
      className="relative flex min-h-screen items-center overflow-hidden bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.18),_transparent_36%),linear-gradient(135deg,_#07111f_0%,_#0f172a_55%,_#111827_100%)] pt-32 pb-28 px-4 sm:px-8"
    >
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-7xl grid-cols-1 items-center gap-12 lg:gap-20 px-4 sm:px-8 lg:px-12 xl:px-16 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          ...
          <div className="mb-6 flex flex-wrap gap-2">
            {portalPills.map((pill) => (
              <span
                key={pill.label}
                className={`rounded-full border px-3 py-1 text-xs font-semibold ${pill.tone}`}
              >
                {pill.label}
              </span>
            ))}
          ...
          <p ref={subRef} className="mt-6 max-w-xl text-base leading-7 text-slate-300">
            SBridge connects students, universities, and recruiters in one calm, structured experience
            so every placement stays visible and moving.
          </p>
          ...
          <div ref={statsRef} className="mt-12 flex flex-wrap gap-4">
            {statCards.map((s) => (
              <div
                key={s.label}
                className={`rounded-2xl border border-slate-800/80 ${s.bg} px-4 py-3 backdrop-blur-sm min-w-[110px]`}
              >
                {s.value === null ? (
                  /* Loading skeleton */
                  <div className="h-7 w-16 rounded-lg bg-slate-700/60 animate-pulse mb-1" />
                ) : (
